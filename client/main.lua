    ...
    CreateThread(function()
        for k in pairs(housePlants[currentHouse]) do
            local plantData = {
                ...
                ['plantProp'] = joaat(TMGWeed.Plants[housePlants[currentHouse][k].sort].stages[housePlants[currentHouse][k].stage]),
            }
...
local function updateHousePlants(leftHouse)
    ...
    CreateThread(function()
        for k in pairs(housePlants[currentHouse]) do
            ...
            for _, stage in pairs(TMGWeed.Plants[housePlants[currentHouse][k].sort]['stages']) do
        ...
        if leftHouse then
            ...
            TMGCore.Functions.TriggerCallback('tmg-weed:server:getBuildingPlants', function(plants)
...
local function getPlantData()
    return {
        ...
        ['plantProp'] = GetHashKey(TMGWeed.Plants[housePlants[currentHouse][closestPlant].sort]['stages'][housePlants[currentHouse][closestPlant].stage]),
        ['plantSort'] = {
            ['name'] = housePlants[currentHouse][closestPlant].sort,
            ['label'] = TMGWeed.Plants[housePlants[currentHouse][closestPlant].sort]['label'],
        },
        ['plantStats'] = {
            ...
            ['highestStage'] = TMGWeed.Plants[housePlants[currentHouse][closestPlant].sort]['highestStage'],
    ...
    CreateThread(function()
        while currentHouse do
            ...
            if plantSpawned and closestPlant ~= 0 then
                ...
                local plantStageLabel = Lang:t('text.stage') .. ' ~b~' .. TMGWeed.StageLabels[plantData['plantStats']['stage']] .. ' ~w~' .. Lang:t('text.progress') .. ' ~b~' .. plantData['plantStats']['progress'] .. '% ~w~ / ' .. Lang:t('text.highestStage') .. ' ~b~' .. TMGWeed.StageLabels[plantData['plantStats']['highestStage']]
                ...
                if plantStageLabel and TMGWeed.ShowStages then drawText3Ds(plantData['plantCoords'].x, plantData['plantCoords'].y, plantData['plantCoords'].z - 0.1, plantStageLabel) end
                ...
                if status > 0 and IsControlJustPressed(0, 38) then
                    ...
                    TMGCore.Functions.Progressbar('remove_weed_plant', Lang:t((status == 1) and 'text.harvesting_plant' or 'text.removing_the_plant'), 8000, false, true, {
                        ...
                        if status == 1 then
                            ...
                            TriggerServerEvent('tmg-weed:server:harvestPlant', currentHouse, amount, plantData['plantSort']['name'], plantData['plantStats']['plantId'])
                        else
                            TriggerServerEvent('tmg-weed:server:removeDeathPlant', currentHouse, plantData['plantStats']['plantId'])
                        end
                    end, function() -- Cancel
                        ClearPedTasks(ped)
...
RegisterNetEvent('tmg-weed:client:getHousePlants', function(house)
    TMGCore.Functions.TriggerCallback('tmg-weed:server:getBuildingPlants', function(plants)
        currentHouse = house
        housePlants[currentHouse] = plants
        spawnHousePlants()
...
RegisterNetEvent('tmg-weed:client:leaveHouse', function()
    updateHousePlants(true)
end)

RegisterNetEvent('tmg-weed:client:refreshHousePlants', function(house)
    if currentHouse == house or not house then
        updateHousePlants(false)
    end
end)

RegisterNetEvent('tmg-weed:client:placePlant', function(type, item)
    if not currentHouse then
        TMGCore.Functions.Notify(Lang:t('error.not_safe_here'), 'error', 3500)
        return
    end
    if closestPlant ~= 0 then
        TMGCore.Functions.Notify(Lang:t('error.cant_place_here'), 'error', 3500)
        return
    end
    local ped = PlayerPedId()
    ...
    local plantData = {
        ...
        ['plantModel'] = TMGWeed.Plants[type]['stages']['stage-a'],
        ['plantLabel'] = TMGWeed.Plants[type]['label']
    }

    LocalPlayer.state:set('inv_busy', true, true)
    TMGCore.Functions.Progressbar('plant_weed_plant', Lang:t('text.planting'), 8000, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        ClearPedTasks(ped)
        TriggerServerEvent('tmg-weed:server:placePlant', json.encode(plantData['plantCoords']), type, currentHouse)
        TriggerServerEvent('tmg-weed:server:removeSeed', item.slot, item.name)
    end, function() -- Cancel
        ClearPedTasks(ped)
        TMGCore.Functions.Notify(Lang:t('error.process_canceled'), 'error')
        LocalPlayer.state:set('inv_busy', false, true)
...
RegisterNetEvent('tmg-weed:client:foodPlant', function()
    ...
    if closestPlant ~= 0 then
        ...
        if plantData['plantStats']['food'] == 100 then
            TMGCore.Functions.Notify(Lang:t('error.not_need_nutrition'), 'error', 3500)
        else
            LocalPlayer.state:set('inv_busy', true, true)
            TMGCore.Functions.Progressbar('plant_weed_plant', Lang:t('text.feeding_plant'), math.random(4000, 8000), false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
                ...
                TriggerServerEvent('tmg-weed:server:foodPlant', currentHouse, newFood, plantData['plantSort']['name'], plantData['plantStats']['plantId'])
            end, function() -- Cancel
                ClearPedTasks(ped)
                LocalPlayer.state:set('inv_busy', false, true)
                TMGCore.Functions.Notify(Lang:t('error.process_canceled'), 'error')
        ...
        TMGCore.Functions.Notify(Lang:t('error.cant_place_here'), 'error')
    end
end)
